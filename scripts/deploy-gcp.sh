#!/usr/bin/env bash
# Deploy novalyte.io (homepage) to Cloud Run on project novalyte-web.
set -euo pipefail

PROJECT_ID="${GCP_PROJECT_ID:-novalyte-web}"
REGION="${GCP_REGION:-us-central1}"
SERVICE="${CLOUD_RUN_SERVICE:-novalyte-homepage}"
...
echo "Deployed. Map domain: gcloud run domain-mappings create --service=${SERVICE} --domain=novalyte.io --region=${REGION} --project=${PROJECT_ID}"
