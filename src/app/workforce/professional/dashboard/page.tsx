  ...
  if (loading) {
    return (
      <WorkspaceShell
        role="professional"
        navItems={[
          { label: "Dashboard", href: "/workforce/professional/dashboard", active: true },
  ...
  if (errorState) {
    return (
      <WorkspaceShell
        ...
        signOutRedirect="/workforce/professional/sign-in"
      >
        <div className="flex flex-1 flex-col items-center justify-center space-y-4 px-4 py-24 text-center">
          <AlertCircle className="h-10 w-10 text-rose-500" />
          <h2 className="text-xl font-bold text-foreground">Error Loading Dashboard</h2>
          <p className="max-w-md text-sm text-muted-foreground">{errorState}</p>
      ...
      {reviewStatus === "pending_review" && (
        <div className="border-b border-amber-200 bg-amber-50 px-4 py-3 text-amber-950">
          <div className="mx-auto flex max-w-7xl items-center gap-2 text-sm">
            ...
            <span>
              <strong>Profile submitted.</strong> Novalyte is reviewing your professional information. Email
              verification does not imply profile approval.
            </span>
          </div>
        </div>
      )}
